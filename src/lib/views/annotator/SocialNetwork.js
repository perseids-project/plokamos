        ...
        var config = {
            ...
            activateFn: (self) => (el) => {
                el.find('div.btn-delete').click((e) => {
                    ...
                    if (triple.closest('.graph.new').length) triple.remove()
                    $('#btn-apply').prop('disabled',self.validator.validate())
                })
                ...
                el.find('input').each((i,e) => $(e).typeahead({minLength:3,highlight:true},{source:self.substringMatcher(self.names),limit:12})) // todo: self
              ...
              <div class="triple" title="Graph:{{g}} Subject:{{s}} Predicate:{{p}} Object:{{o}}" data-original-subject="{{s}}" data-original-predicate="{{p}}" data-original-object="{{o}}" data-subject="{{s}}" data-predicate="{{p}}" data-object="{{o}}">
                <div class="sentence well container">
                  ...
                  <div class="token predicate col-xs-12 col-md-4" data-token="predicate">
                      <div class="input-group">
                          <span class="input-group-addon" title="Person ID" id="basic-addon2"><span class="glyphicon glyphicon-random"></span></span>
                  ...
                  <div class="token object col-xs-12 col-md-4" data-token="object">
                      <div class="input-group">
                          <span class="input-group-addon" title="Person ID" id="basic-addon3"><span class="glyphicon glyphicon-user"></span></span>
                    <input class="typeahead" placeholder="Object" value="{{#label}}{{o}}{{/label}}">
                    </div>
                  </div>
                </div>
                <div class="btn-delete btn btn-danger" title="Delete triple"><span class="glyphicon glyphicon-trash"/></div>
              </div>
            `
            },
            validateMp: {
            ...
            validateFn: (map) => (token, text) => (token && text) ?
                _.reduce(map[token], (acc, x) => acc || text.startsWith(x), false) :
                $('.graph.old').find('.invalid').length
                || $('.graph.new').find('.typeahead.tt-input.valid').length!=$('.graph.new').find('.typeahead.tt-input').length
                || $('.triple.update').length+$('.triple.delete').length==0
        }
